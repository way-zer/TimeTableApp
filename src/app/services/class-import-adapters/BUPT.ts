...
export default class BUPTParser implements ClassImportAdapter {
  ...
  parse(dom: HTMLElement): Class[] {
    ...
    formatted.forEach((row: string[]) => {
      const c = map[row[2]] || new Class(row[2], +row[4], row[7], row[17], []);
      const regex = /(|.*：)(\d+)-(\d+)/;
      const weekI = row[11].match(regex);
      if (weekI) {
        const time = new ClassTime(new Range(0, 0), +row[12],
          new Range(+row[13], (+row[13]) + (+row[14]) - 1), c.place !== row[17] && row[17]);
